...
func newMovementSetup(t *testing.T, width, height, winLen int) (*service.Movement, *model.Session) {
	t.Helper()
	rules := &model.Rules{UUID: "rules-1", BoardWidth: width, BoardHeight: height, WinLength: winLen}
	checker := service.NewWinCheckerService(rules)
	movement := service.NewMovementService(checker)
	session := model.NewSession("session-1", &model.SessionParams{Seed: 0}, rules)
	return movement, session
}

func newPlayerWithMark(uuid string, mark model.Mark) *model.Player {
	p := model.NewPlayer(uuid, "player-"+uuid)
	p.Mark = mark
	return p
}
...
func TestMakeOutOfBounds(t *testing.T) {
	...
	err := movement.Make(session, playerX, geometry.NewPoint(10, 10))
...
func TestMakeCellOccupied(t *testing.T) {
	...
	require.NoError(t, movement.Make(session, playerX, geometry.NewPoint(0, 0)))
	err := movement.Make(session, playerO, geometry.NewPoint(0, 0))
...
func TestMakeValidMoveGameContinues(t *testing.T) {
	...
	err := movement.Make(session, playerX, geometry.NewPoint(0, 0))

	require.NoError(t, err)
	assert.Equal(t, model.StatePlaying, session.State)
...
func TestMakeTurnIncrements(t *testing.T) {
	...
	require.NoError(t, movement.Make(session, playerX, geometry.NewPoint(0, 0)))
	require.NoError(t, movement.Make(session, playerO, geometry.NewPoint(1, 0)))
	require.NoError(t, movement.Make(session, playerX, geometry.NewPoint(2, 0)))
...
func TestMakeXWins(t *testing.T) {
	...
	err := movement.Make(session, playerX, geometry.NewPoint(0, 2))
...
func TestMakeOWins(t *testing.T) {
	...
	err := movement.Make(session, playerO, geometry.NewPoint(2, 0))
...
func TestMakeDraw(t *testing.T) {
	...
	err := movement.Make(session, playerO, geometry.NewPoint(1, 1))
...
func TestMakeAfterGameOver(t *testing.T) {
	...
	require.NoError(t, movement.Make(session, playerX, geometry.NewPoint(0, 2)))
	require.Equal(t, model.StateGameOver, session.State)
	turnAfterWin := session.Turn

	err := movement.Make(session, playerX, geometry.NewPoint(1, 0))
